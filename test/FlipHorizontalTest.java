...
/**
 * Test for {@link FlipHorizontal}s.
 */
public class FlipHorizontalTest {
  ...
  public void getNewCoordinates() {
    ...
    IImageMutable flipHorizontal = new FlipHorizontal().process(this.image);
    for (int row = 0; row < flipHorizontal.getHeight(); row++) {
      for (int col = 0; col < flipHorizontal.getWidth(); col++) {
        assertEquals(this.image.getPixel(row, col),
                flipHorizontal.getPixel(row, (flipHorizontal.getWidth() - col) - 1));
      }
    }
  }
  ...
  public void testToString() {
    assertEquals("horizontally flipped", new FlipHorizontal().toString());
  }
